...
if view == "MIS":
    ...
    st.subheader("Stuck deals — Created-in-window, split by Trial status")

    # Reuse globally filtered df (df_f) and mapped columns you already computed
    if not create_col:
        st.error("Create Date column is required for this view.")
        st.stop()

    # Date scope (based on Create Date)
    ...
    if scope_choice == "This month":
        range_start, range_end = month_bounds(today)
        st.caption(f"Scope: **This month** ({range_start} → {range_end})")
    elif scope_choice == "Last month":
        range_start, range_end = last_month_bounds(today)
        st.caption(f"Scope: **Last month** ({range_start} → {range_end})")
    else:
        c1, c2 = st.columns(2)
        with c1:
            range_start = st.date_input("Start date", value=today.replace(day=1), key="stuck_start")
        with c2:
            range_end = st.date_input("End date", value=month_bounds(today)[1], key="stuck_end")
        if range_end < range_start:
            st.error("End date cannot be before start date.")
            st.stop()
        st.caption(f"Scope: **Custom** ({range_start} → {range_end})")
    ...
    status_mode = st.radio(
        "Trial status",
        ["TND (Trial not done)", "TD (Trial done)"],
        index=0,
        horizontal=True,
        key="stuck_mode",
        help="TND = Calibration Done Date is empty; TD = Calibration Done Date present. Population is deals created in the selected window.",
    )
    ...
    # Calibration Done column handling
    if cal_done_col and cal_done_col in d.columns:
        d["_cal_done_dt"] = coerce_datetime(d[cal_done_col]).dt.date
    else:
        d["_cal_done_dt"] = pd.NaT
        st.warning("Calibration Done column not found; all records will be treated as TND.", icon="⚠️")

    # Filter by Create Date window
    in_window = d["_create_dt"].between(range_start, range_end)

    # Apply status logic
    if status_mode.startswith("TND"):
        status_mask = d["_cal_done_dt"].isna()
    else:
        status_mask = d["_cal_done_dt"].notna()

    stuck_df = d.loc[in_window & status_mask].copy()

    # KPIs
    total_created_in_window = int(in_window.sum())
    stuck_count = int(len(stuck_df))
    pct_of_created = (stuck_count / total_created_in_window * 100.0) if total_created_in_window > 0 else 0.0

    k1, k2, k3 = st.columns(3)
    with k1:
        st.markdown(
            f"<div class='kpi-card'><div class='kpi-title'>Deals created (window)</div>"
            f"<div class='kpi-value'>{total_created_in_window:,}</div>"
            f"<div class='kpi-sub'>{range_start} → {range_end}</div></div>",
            unsafe_allow_html=True,
        )
    with k2:
        st.markdown(
            f"<div class='kpi-card'><div class='kpi-title'>{'TND' if status_mode.startswith('TND') else 'TD'} count</div>"
            f"<div class='kpi-value'>{stuck_count:,}</div>"
            f"<div class='kpi-sub'>Create-based population</div></div>",
            unsafe_allow_html=True,
        )
    with k3:
        st.markdown(
            f"<div class='kpi-card'><div class='kpi-title'>Share of created</div>"
            f"<div class='kpi-value'>{pct_of_created:.1f}%</div>"
            f"<div class='kpi-sub'>Num: {stuck_count:,} • Den: {total_created_in_window:,}</div></div>",
            unsafe_allow_html=True,
        )

    # Grouped quick views (optional)
    grp_cols = []
    if source_col: grp_cols.append(source_col)
    if country_col: grp_cols.append(country_col)
    if counsellor_col: grp_cols.append(counsellor_col)

    if stuck_df.empty:
        st.info("No rows match the selected scope and status.")
    else:
        st.markdown("#### Details")
        show_cols = []
        for c in [create_col, first_cal_sched_col, cal_resched_col, cal_done_col, pay_col, source_col, country_col, counsellor_col]:
            if c and c in stuck_df.columns: show_cols.append(c)

        # Nicely sorted preview
        preview = stuck_df[show_cols].copy() if show_cols else stuck_df.copy()
        if cal_done_col and cal_done_col in preview.columns:
            preview = preview.sort_values(by=[cal_done_col, create_col], na_position="first")
        else:
            preview = preview.sort_values(by=[create_col])

        st.dataframe(preview.head(1000), use_container_width=True)

        # Downloads
        st.download_button(
            "Download CSV – Stuck deals (current view)",
            preview.to_csv(index=False).encode("utf-8"),
            file_name=f"stuck_deals_{'TND' if status_mode.startswith('TND') else 'TD'}_{range_start}_{range_end}.csv",
            mime="text/csv",
            key="stuck_dl_csv",
        )

        # Quick group summaries
        with st.expander("Group summaries (optional)"):
            if grp_cols:
                for g in grp_cols:
                    gtbl = (stuck_df.assign(_one=1)
                                   .groupby(g, dropna=False)["_one"]
                                   .sum().reset_index()
                                   .rename(columns={"_one":"Count"}))
                    gtbl = gtbl.sort_values("Count", ascending=False)
                    st.markdown(f"**By {g}**")
